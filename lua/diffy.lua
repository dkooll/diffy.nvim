...
--[[ HCL Parsing and Validation Core ]] --
...
local function parse_block_contents(node, bufnr)
  ...
  for _, match in attr_query:iter_matches(node, bufnr) do
  ...
  for _, match in block_query:iter_matches(node, bufnr) do
    local name = vim.treesitter.get_node_text(match[1], bufnr)
    local body = match[2]

    if name == "lifecycle" then
      block_data.ignore_changes = parse_ignore_changes_array(body, bufnr)
    elseif name ~= "dynamic" then
      block_data.blocks[name] = parse_block_contents(body, bufnr)
    end
  end
  ...
  for _, match in dynamic_query:iter_matches(node, bufnr) do
    local dyn_name = vim.treesitter.get_node_text(match[2], bufnr):gsub('"', "")
    local dyn_body = match[3]
    ...
    for _, cmatch in content_query:iter_matches(dyn_body, bufnr) do
      content_data = parse_block_contents(cmatch[2], bufnr)
    end

    block_data.dynamic_blocks[dyn_name] = content_data or parse_block_contents(dyn_body, bufnr)
  end
...
--[[ Submodule Handling ]] --
local function find_submodules(root_dir)
  local modules_dir = root_dir .. "/modules"
  local submodules = {}

  if vim.fn.isdirectory(modules_dir) == 1 then
    for _, dir in ipairs(vim.fn.readdir(modules_dir)) do
      local sub_path = modules_dir .. "/" .. dir
      local main_tf = sub_path .. "/main.tf"
...
local function fetch_schema_for_directory(dir_path, callback)
  ...
  -- Copy required Terraform files
  for _, fname in ipairs({ "terraform.tf", "main.tf" }) do
    local src = dir_path .. "/" .. fname
    if vim.fn.filereadable(src) == 1 then
      vim.fn.system({ "cp", src, temp_dir .. "/" .. fname })
      if vim.v.shell_error ~= 0 then
        write_output("Failed to copy " .. fname)
        cleanup(temp_dir)
        return
      end
    end
  ...
  vim.fn.jobstart({ "terraform", "init" }, {
    cwd = temp_dir,
    on_exit = function(_, exit_code)
      if exit_code ~= 0 then
        write_output("Init failed in " .. dir_path)
        cleanup(temp_dir)
        return
      end
      ...
      vim.fn.jobstart({ "terraform", "providers", "schema", "-json" }, {
        ...
        on_stdout = function(_, data)
          ...
          if success and decoded and decoded.provider_schemas then
            callback(decoded.provider_schemas)
          end
        end,
        on_exit = function() cleanup(temp_dir) end
      })
    end
  })
end
...
--[[ Validation Core ]] --
local function validate_block(resource_type, schema, data, path, ignores, results)
  ignores = vim.tbl_extend("keep", ignores or {}, data.ignore_changes)

  -- Validate attributes
  for name, attr in pairs(schema.attributes or {}) do
    if not attr.computed and not vim.tbl_contains(ignores, name) and not data.properties[name] then
      table.insert(results, string.format(
        "%s: Missing %s attribute '%s' at %s",
        resource_type,
        attr.required and "required" or "optional",
        name,
        path
      ))
    end
  ...
  -- Validate blocks
  for name, block_schema in pairs(schema.block_types or {}) do
    if name == "timeouts" then goto continue end

    local static = data.blocks[name]
    local dynamic = data.dynamic_blocks[name]
    local child_path = path .. "." .. name

    if not static and not dynamic then
      table.insert(results, string.format(
        "%s: Missing %s block '%s' at %s",
        resource_type,
        block_schema.min_items > 0 and "required" or "optional",
        name,
        path
      ))
    else
      local target = static or dynamic
      validate_block(
        resource_type,
        block_schema.block,
        target,
...
function M.validate_resources()
  ...
  local submodules = find_submodules(root_dir)
  local results = {}
  local pending = 1 + #submodules

  local function process_directory(dir_path)
    fetch_schema_for_directory(dir_path, function(schemas)
      ...
      local query = vim.treesitter.query.parse("hcl", [[
        (block
          (identifier) @type (#eq? @type "resource")
      ...
      for _, match in query:iter_matches(tree:root(), 0) do
        local resource_type = vim.treesitter.get_node_text(match[2], 0):gsub('"', "")
        local body = match[4]
        table.insert(resources, {
          type = resource_type,
          data = parse_block_contents(body, 0)
        })
      end

      -- Schema validation
      for _, resource in ipairs(resources) do
        local provider = resource.type:match("^(%w+)_")
        local provider_schema = schemas["registry.terraform.io/" .. provider] or schemas[provider]

        if provider_schema and provider_schema.resource_schemas then
          local schema = provider_schema.resource_schemas[resource.type]
          if schema then
            validate_block(
              resource.type,
              schema.block,
            ...
            table.insert(results, "No schema for " .. resource.type)
          end
        else
          table.insert(results, "Provider not found for " .. resource.type)
        end
      end
  ...
  -- Process root and all submodules
  process_directory(root_dir)
  for _, sub in ipairs(submodules) do
    process_directory(sub.path)
  end
end
