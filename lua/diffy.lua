...
local function write_output(lines, clear)
  ...
  vim.api.nvim_buf_set_lines(buf, line_count, line_count, false, lines or {})

  local win = ensure_output_window()
  vim.api.nvim_win_set_cursor(win, { line_count + #(lines or {}), 0 })
  vim.cmd("redraw")
end
...
-- Refactored fetch_schema function:
-- Runs "terraform init" and "terraform providers schema -json" in the current directory.
-- Caches schemas from all providers found.
    ...
    on_exit = function(_, exit_code)
      ...
      vim.fn.jobstart({ "terraform", "providers", "schema", "-json" }, {
        ...
        on_stderr = function(_, err_data)
          if err_data and #err_data > 0 then
            write_output(vim.tbl_filter(function(line)
              return line and line ~= ""
            end, err_data))
          end
...
-- Gather item names from lifecycle.ignore_changes.
...
function M.validate_resources()
  ...
  M.fetch_schema(function()
    ...
    for _, resource in ipairs(resources) do
      local found_schema
      -- Search all loaded providers for the resource type
      for _, provider_data in pairs(schema_cache) do
        local maybe_schema = provider_data.resource_schemas
          and provider_data.resource_schemas[resource.type]
        if maybe_schema then
          found_schema = maybe_schema
          break
        end
      end
      if not (found_schema and found_schema.block) then
      ...
      validate_block_attributes(found_schema.block, resource, "root", resource.ignore_changes)
      ::continue_resources::
    end
  end)
end
