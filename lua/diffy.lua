...
------------------------------------------------------
-- SCHEMA FETCH (unchanged)
------------------------------------------------------
local function fetch_schema(callback)
  write_output({}, true)
...
local function do_schema_validation_for_resources(resources, cache, unique_messages)
  local used_provider_keys = {}

  for _, resource in ipairs(resources) do
    local matching_provider_block = nil

    for provider_key, provider_data in pairs(cache) do
  ...
  for provider_key, _ in pairs(cache) do
...
------------------------------------------------------
-- HELPER: find submodules
------------------------------------------------------
local function find_submodules_one_level(root_dir)
  local modules_dir = root_dir .. "/modules"
  local result = {}

  local ok, items = pcall(vim.loop.fs_scandir, modules_dir)
  if not ok or not items then
    return result
  end

  while true do
    local name, t = vim.loop.fs_scandir_next(items)
    if not name then
      break
    end
    if t == "directory" then
      -- check main.tf
      local main_path = modules_dir .. "/" .. name .. "/main.tf"
      local f = io.open(main_path, "r")
      if f then
        f:close()
        table.insert(result, modules_dir .. "/" .. name)
      end
    end
  end
...
local function parse_file_with_treesitter(filename)
...
local function validate_submodules(root_dir, messages)
  local sub_dirs = find_submodules_one_level(root_dir)
  if #sub_dirs == 0 then
    table.insert(messages, "No submodules found under " .. root_dir .. "/modules")
    return
  end

  for _, smdir in ipairs(sub_dirs) do
    local main_file = smdir .. "/main.tf"
    local tf_file = smdir .. "/terraform.tf"
    local tf_handle = io.open(tf_file, "r")
    if not tf_handle then
      table.insert(messages, "Skipping submodule (no terraform.tf): " .. smdir)
    else
      tf_handle:close()
      -- parse main.tf
      local sub_resources = parse_file_with_treesitter(main_file)
      if #sub_resources == 0 then
        table.insert(messages, "No resources found in submodule: " .. smdir)
      end

      -- same approach: temp dir, copy terraform.tf, run init, fetch schema, validate
      local temp_dir = create_temp_dir()
      if not temp_dir then
        table.insert(messages, "Failed to create temp dir for submodule " .. smdir)
        goto continue_sub
      end

      vim.fn.system({ "cp", tf_file, temp_dir .. "/terraform.tf" })
      if vim.v.shell_error ~= 0 then
        table.insert(messages, "Failed to copy terraform.tf to temp dir for " .. smdir)
        cleanup(temp_dir)
        goto continue_sub
      end

      local init_out = vim.fn.systemlist({ "terraform", "init" }, temp_dir)
      if vim.v.shell_error ~= 0 then
        vim.list_extend(messages, init_out)
        table.insert(messages, "Failed to terraform init in submodule " .. smdir)
        cleanup(temp_dir)
        goto continue_sub
      end

      local schema_out = vim.fn.systemlist({ "terraform", "providers", "schema", "-json" }, temp_dir)
      if vim.v.shell_error ~= 0 then
        vim.list_extend(messages, schema_out)
        table.insert(messages, "Failed to fetch schema for submodule: " .. smdir)
        cleanup(temp_dir)
        goto continue_sub
      end

      local json_str = table.concat(schema_out, "\n")
      local ok, decoded = pcall(vim.json.decode, json_str)
      if not ok or not decoded or not decoded.provider_schemas then
        table.insert(messages, "Failed to parse schema JSON for " .. smdir)
        cleanup(temp_dir)
        goto continue_sub
      end

      local sub_schema = decoded.provider_schemas
      local unique_sub = {}
      do_schema_validation_for_resources(sub_resources, sub_schema, unique_sub)
      for msg in pairs(unique_sub) do
        table.insert(messages, msg .. " [submodule: " .. smdir .. "]")
      end
...
------------------------------------------------------
-- MAIN: validate_resources => same command
------------------------------------------------------
function M.validate_resources()
  write_output({}, true)

  -- 1) Validate the current buffer as before
  fetch_schema(function()
    ...
    -- print root result
    if #messages == 0 then
      write_output("No issues in the current buffer.")
    else
      write_output(messages)
    end

    -- 2) Then validate submodules in the same command
    write_output({ "", "-- Validating submodules --" })
    local root_dir = vim.fn.getcwd()
    validate_submodules(root_dir, messages) -- we append submodule findings to the same table

    if #messages == 0 then
      write_output("No submodule findings either.")
    else
      -- Show the final merged list (root + submodules).
      -- We already printed root messages, so just print submodule lines.
      -- If we want them sorted, do so:
      table.sort(messages)
      write_output(messages)
    end
  end)
