...
local function get_subdirectories(path)
  local result = {}
  local handle = io.popen("find " .. path .. " -maxdepth 1 -type d -not -path " .. path)
  if handle then
...
-- Check if a directory has a terraform.tf file
local function has_terraform_file(dir)
  return vim.fn.filereadable(dir .. "/terraform.tf") == 1
end

-- Process a single module directory
local function process_module(module_path, callback)
  write_output({ "", "Processing module: " .. module_path })

  local temp_dir = create_temp_dir()
  if not temp_dir then
    write_output("Failed to create temporary directory for module: " .. module_path)
    return false
  end

  local terraform_tf_path = module_path .. "/terraform.tf"
  if vim.fn.filereadable(terraform_tf_path) == 0 then
    write_output("Could not find terraform.tf in " .. module_path)
    cleanup(temp_dir)
    return false
  end

  vim.fn.system({ "cp", terraform_tf_path, temp_dir .. "/terraform.tf" })
  if vim.v.shell_error ~= 0 then
    write_output("Failed to copy terraform.tf from module to temp directory.")
    cleanup(temp_dir)
    return false
  end
  ...
  local init_job = vim.fn.jobstart({ "terraform", "init" }, {
    ...
    on_exit = function(_, exit_code)
      if exit_code ~= 0 then
        write_output("Failed to initialize Terraform for module: " .. module_path)
        cleanup(temp_dir)
        return
      end
      ...
      vim.fn.jobstart({ "terraform", "providers", "schema", "-json" }, {
        ...
        on_stdout = function(_, data)
          if data and #data > 0 then
            ...
            if success and decoded and decoded.provider_schemas then
              -- Merge schemas from this module with the main schema cache
              for provider_key, provider_data in pairs(decoded.provider_schemas) do
                schema_cache[provider_key] = provider_data
              end
              if callback then
                callback()
              end
            else
              write_output("Failed to parse schema JSON for module: " .. module_path)
            end
          end
        end,
        on_stderr = function(_, data)
        ...
        on_exit = function(_, schema_exit_code)
          if schema_exit_code ~= 0 then
            write_output("Failed to fetch schema for module: " .. module_path)
          end
          cleanup(temp_dir)
        end
      })
  ...
  if init_job == 0 then
    write_output("Failed to start Terraform initialization for module: " .. module_path)
    cleanup(temp_dir)
    return false
  end
...
local function process_submodules(callback)
  local current_dir = vim.fn.getcwd()
  local modules_dir = current_dir .. "/modules"

  if vim.fn.isdirectory(modules_dir) == 0 then
    write_output("No modules directory found.")
    callback()
    return
  end

  local submodules = get_subdirectories(modules_dir)
  if #submodules == 0 then
    write_output("No submodules found in modules directory.")
    callback()
    return
  end

  write_output("Found " .. #submodules .. " submodules.")

  -- Filter submodules that have terraform.tf
  local valid_submodules = {}
  for _, submodule in ipairs(submodules) do
    if has_terraform_file(submodule) then
      table.insert(valid_submodules, submodule)
    end
  end

  if #valid_submodules == 0 then
    write_output("No submodules with terraform.tf found.")
    callback()
    return
  end

  write_output(#valid_submodules .. " submodules have terraform.tf files.")

  -- Process submodules one by one
  local current_index = 1

  local function process_next_submodule()
    if current_index > #valid_submodules then
      write_output("Finished processing all submodules.")
      callback()
      return
    end

    local submodule = valid_submodules[current_index]
    current_index = current_index + 1

    process_module(submodule, process_next_submodule)
  end

  process_next_submodule()
end

function M.fetch_schema(callback)
  write_output({}, true)
  schema_cache = {} -- Reset schema cache
        ...
        on_stdout = function(_, data)
          if data and #data > 0 then
            ...
            if success and decoded and decoded.provider_schemas then
              ...
              -- After processing the main directory, process submodules
              process_submodules(function()
                if callback then
                  write_output({ "" })
                  callback()
                end
