...
local function process_module(module_path)
  ...
  local init_job = vim.fn.jobstart({ "terraform", "init" }, {
    ...
    on_stdout = function(_, _)
      -- Suppressed terraform init output
    end,
    on_stderr = function(_, data)
      if data and #data > 0 then
        -- Only show actual errors, not normal stderr output
        local errors = vim.tbl_filter(function(line)
          -- Filter to only show real errors, not normal terraform output
    ...
    on_exit = function(_, exit_code)
      ...
      vim.fn.jobstart({ "terraform", "providers", "schema", "-json" }, {
        ...
        on_stdout = function(_, data)
          -- Process data silently without output
          if data and #data > 0 then
        ...
        on_exit = function(_, schema_exit_code)
          ...
          -- Clean up the .terraform directory after we're done
...
function M.validate_resources()
  write_output({}, true)
  write_output("Validating Terraform resource schemas...")
  global_messages = {}
