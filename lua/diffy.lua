...
function M.fetch_schema(callback)
  ...
  -- Copy local terraform.tf into the temp dir (to avoid cluttering main folder)
  ...
  local init_job = vim.fn.jobstart({ "terraform", "init" }, {
    ...
    on_exit = function(_, exit_code)
      ...
      -- Once "terraform init" succeeds, fetch **all** provider schemas
...
function M.validate_resources()
  ...
  M.fetch_schema(function()
    ...
    -- We'll keep track of which providers are actually used by resources
    ...
    for _, resource in ipairs(resources) do
      ...
      -- We'll scan all loaded providers for a matching resource type
      for provider_key, provider_data in pairs(schema_cache) do
        ...
        if r_schemas and r_schemas[resource.type] then
          ...
          matching_provider_key = provider_key
      ...
      if not (matching_provider_block and matching_provider_key) then
        write_output("No provider schema found for resource: " .. resource.type)
      else
        -- If we found a matching provider & block, do normal validation
        local function validate_block_attributes(block_schema, block_data, block_path, inherited_ignores)
            ...
            for attr_name, attr_info in pairs(block_schema.attributes) do
              ...
              if not attr_info.computed and not block_data.properties[attr_name] then
                if attr_info.required then
                  ...
                  write_output(
                    string.format(
                      "%s missing optional property %s in path %s",
                      resource.type, attr_name, block_path
                    )
                  )
                end
          ...
          if block_schema.block_types then
            for block_name, btype_schema in pairs(block_schema.block_types) do
              -- skip "timeouts" block or other internal blocks if you like
              ...
              elseif dyn_block then
                ...
                if btype_schema.min_items and btype_schema.min_items > 0 then
                  write_output(
                    string.format(
                      "%s missing required block %s in path %s",
                  ...
                  write_output(
                    string.format(
                      "%s missing optional block %s in path %s",
                      resource.type, block_name, block_path
                    )
                  )
                end
        ...
        -- Start validation on the resource "root"
        validate_block_attributes(matching_provider_block, resource, "root", resource.ignore_changes)
      end
    end

    for provider_key, _ in pairs(schema_cache) do
      if not used_provider_keys[provider_key] then
        write_output("Provider declared but not used by any resource: " .. provider_key)
      end
    end
  end)
end
